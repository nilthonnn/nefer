"""Interfaz de linea de comandos de rdrenta."""
...
def cmd_catalogo(args) -> int:
    ...
    if args.crear:
        ...
        print("Complete sus clientes y equipos y vuelva a usar `rdrenta acta`.")
        return 0
...
def construir_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--version", action="version", version=f"rdrenta {__version__}")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        # `rdrenta fotos | head` cierra la salida antes de tiempo; no es un error.
        try:
            sys.stdout.close()
        except Exception:
            pass
