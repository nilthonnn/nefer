...
Cómo levantar un acta de despacho o recepción con `rdrenta`, desde la primera
...
```bash
git clone https://github.com/nilthonnn/rd-renta.git
cd rd-renta
pip install -e .
```
...
- **La app se abrió como archivo local.** Medido: desde `file://` el permiso de
  ...
  Abra **https://nilthonnn.github.io/nefer/rd-rental/app/**.
...
1. Abra **https://nilthonnn.github.io/nefer/rd-rental/app/** en el navegador del celular.
...
```bash
rdrenta acta -e GE110-02 -c andina --acta 004-001120 --guia T001-00019876
```
...
`~/.rdrenta/catalogo.json`, en ese orden: el último sirve como maestro compartido
de toda la flota.

`rdrenta catalogo` lo lista, y busca por fragmento: `-e ge110`, `-e torre`,
...
Abra la aplicación —en el celular desde **https://nilthonnn.github.io/nefer/rd-rental/app/**, en la computadora también
...
caen en las mismas filas que `rdrenta construir`.
...
implementación del mismo formato que produce `rdrenta construir`: la geometría se
copió de `rdrenta/layout.py` y hay pruebas que comparan ambas listas de constantes
...
```bash
rdrenta fotos fotos/ -m acta.json
```
...
```bash
rdrenta construir acta.json -o salidas/TI009-04-DESPACHO.xlsx --pdf
```
...
```bash
rdrenta extraer acta-2025.xlsx -o acta-2025.json --fotos fotos-2025
```
...
| Comando | Qué hace |
|---|---|
| `rdrenta catalogo --crear` | Crea el maestro de clientes y equipos |
| `rdrenta catalogo` | Lista lo que hay en el catálogo |
| `rdrenta acta -e GE110-02 -c andina` | Acta con el encabezado ya lleno |
| `rdrenta plantilla -o acta.json` | Manifiesto en blanco, sin catálogo |
| `rdrenta fotos fotos/ -m acta.json` | Carga la carpeta de fotos en el manifiesto |
| `docs/app/` | Aplicación de campo: despacho y recepción, sin conexión |
| `rdrenta validar acta.json` | Verifica el manifiesto y que las fotos existan |
| `rdrenta validar acta.json --sin-verificar-fotos` | Solo el manifiesto, sin mirar el disco |
| `rdrenta construir acta.json -o salida.xlsx` | Genera el Excel |
| `rdrenta construir acta.json -o salida.xlsx --pdf` | Excel y PDF |
| `rdrenta construir ... --sin-guias` | Sin las hojas de guía |
| `rdrenta extraer acta.xlsx -o acta.json --fotos fotos/` | Excel llenado → manifiesto |
| `rdrenta pdf salida.xlsx` | Convierte un Excel ya generado |
| `rdrenta guias -o docs/` | Exporta las guías en Markdown |
...
Tanto `rdrenta fotos` como el generador avisan y dejan el bloque vacío en vez de
fallar en silencio.
