...
dos veces, en `rdrenta/layout.py` y en el JavaScript de la aplicación. Está
...
| `ultima_fila()` | `rdrenta/layout.py` | sin ninguna llamada |
...
`cmd_*` de `rdrenta/cli.py` los despacha argparse, no una llamada directa.
...
`rdrenta/layout.py` y el JavaScript de la aplicación describen la misma rejilla.
