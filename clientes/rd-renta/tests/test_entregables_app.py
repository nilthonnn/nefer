...
Es una segunda implementacion del mismo formato que produce `rdrenta construir`.
...
from rdrenta import build, layout
...
def test_el_excel_pagina_como_el_formato(entregables, tmp_path):
    ...
    Los cortes se comparan con los que pone `rdrenta construir` sobre el mismo
    acta: si la regla cambia alli, esta prueba lo dice.
    """
    from rdrenta import schema
...
def test_la_recepcion_cae_en_las_mismas_filas_que_el_escritorio(recepcion, tmp_path):
    ...
    from rdrenta import build as _build, schema
...
def test_el_archivo_descargable_es_la_misma_app_que_esta_publicada(tmp_path):
    ...
    `docs/rdrenta-app.html` es la app entera en un archivo, y se sirve desde la
    ...
    descargable = APP.parent.parent / "rdrenta-app.html"
    assert descargable.exists(), "falta docs/rdrenta-app.html"
    ...
    assert descargable.read_bytes() == recien.read_bytes(), (
        "docs/rdrenta-app.html no coincide con la app: "
        "rehacerlo con `python herramientas/empaquetar-demo.py docs/rdrenta-app.html`")
...
def test_el_qr_apunta_a_la_direccion_publicada():
    ...
    assert 'href="rdrenta-app.html"' in portada, "y ofrecer la descarga del archivo"
...
def test_la_cabecera_ocupa_las_columnas_del_formato(recepcion, tmp_path):
    ...
    from rdrenta import build as _build
