...
VISTAS_POR_CATEGORIA = {
    ...
    # Vehiculo liviano (rent-a-car / flota corporativa): la metrica de uso es
    # el odometro, no el horometro. Ver ETIQUETA_METRICA_POR_CATEGORIA.
    "vehiculo_liviano": [
        "VISTA FRONTAL", "VISTA POSTERIOR",
        "VISTA LATERAL IZQUIERDA", "VISTA LATERAL DERECHA",
        "ODÓMETRO / TABLERO", "INTERIOR",
        "LLANTAS",
    ],
}

# Etiqueta que va impresa junto al valor de la fila 9 (celda CELDA_HOROMETRO).
# El campo del manifiesto sigue llamandose `horometro` por compatibilidad con
# las actas y las pruebas existentes: lo que cambia por categoria es solo el
# rotulo impreso, no el nombre del campo ni su validacion (numero, o la cadena
# REVISION_MANUAL si es ilegible).
ETIQUETA_METRICA_POR_CATEGORIA = {
    "vehiculo_liviano": "ODÓMETRO",
}
ETIQUETA_METRICA_DEFECTO = "HORÓMETRO"


def etiqueta_metrica(categoria: str | None) -> str:
    """Rotulo de la metrica de uso para `categoria` ('HORÓMETRO' si no hay
    entrada especifica, que es el comportamiento de siempre)."""
    return ETIQUETA_METRICA_POR_CATEGORIA.get(categoria, ETIQUETA_METRICA_DEFECTO)
...
PISTAS_NOMBRE = [
    ...
    ("MOTOR", ("motor", "engine")),
    ("ODÓMETRO / TABLERO", ("odometro", "odómetro", "kilometraje", "dashboard", "odometer")),
    ("INTERIOR", ("interior", "asientos", "cabina interior")),
]
